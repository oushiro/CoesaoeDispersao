...
# 											#
# LO: Minhas inserções estão comentadas em Sentence case                                #
#########################################################################################

# PROMPT THE USER FOR LOCATION OF INPUT/OUTPUT FILES, FORMANT SETTINGS, ETC.
form Vowel Analyzer
	comment Paths:
	sentence Soundfile_directory C:/Users/XPS/Dropbox/_PESQUISAS/PROJETO_COESAOeDISPERSAO/Corpus_ProjetoCoesaoeDispersao/_ProjetoCeD_CorpusFinal/output_AmostraMourao/output_JuntarTrilhas/feminino/
	sentence Textgrid_directory C:/Users/XPS/Dropbox/_PESQUISAS/PROJETO_COESAOeDISPERSAO/Corpus_ProjetoCoesaoeDispersao/_ProjetoCeD_CorpusFinal/output_AmostraMourao/output_JuntarTrilhas/feminino/
	sentence Results_file vogais_AmostraMourao-F.txt
	optionmenu Use_targets_file 1
		option yes
		option no
	sentence Targets_file palavrasPret-pDeepSeek.txt
	comment Tiers:
	sentence Vowel_tier phones
	optionmenu Use_word_tier: 1
		option yes
		option no
	sentence Word_tier words
	optionmenu Use_notes_tier: 1
		option yes
		option no
	sentence Notes_tier Pret
	comment Formant settings:
	optionmenu Measurement_points: 2
		option Midpoint
		option 30%/50%/70%
		option 25%/50%/75%
		option 20%/50%/80%
#para homens, 5000, 5 formantes; para mulheres, 5500, 5 formantes
...
for ifile to numberoffiles
	...
	if fileReadable (gridfile$)
		...
		for interval to intervals - 1
			...
			# Get next phone for following contexts
			nextPhoneme$ = Get label of interval... vowel_tier interval + 1

			# Get preceding phone for preceding contexts
			if interval > 1
    				precPhoneme$ = Get label of interval... vowel_tier interval - 1
			else
				precPhoneme$ = ""  
			endif
			...
			# Find out if the phoneme is a vowel; if so, proceed
			if  index_regex(phone_label$, "(a|e|i|o|u|ɔ|ɛ|ɐ)")
				...
				# CREATE RESULTS LINE
				if use_word_tier = 1
					resultsline_begin$ = "'soundfile$'	'word_label$'	'precPhoneme$'	'phone_label$'	'nextPhoneme$'	'start'	'end'	'duration_ms'	"
				else
					resultsline_begin$ = "'soundfile$'	'precPhoneme$'	'phone_label$'	'nextPhoneme$'	'start'	'end'	'duration_ms'	"
				endif
				if use_notes_tier = 1
...
procedure InitializeResultsFile
	if use_word_tier = 1
		header_begin$ = "ARQUIVO	Word	precPhon	Vowel	nextPhon	beginTime_s	endTime_s	duration_ms	"
	else
		header_begin$ = "ARQUIVO	precPhon	Vowel	nextPhon	beginTime_s	endTime_s	duration_ms	"
	endif
	if use_notes_tier = 1
		header_end$ = "Pret	Analyst	Date	Settings	PraatVersion	OS'newline$'"
	else
		header_end$ = "Analyst	Date	Settings	PraatVersion	OS'newline$'"
	endif
	if measurement_points = 1
